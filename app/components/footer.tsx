...
const Footer = () => {
  return (
      <footer className={"flex flex-col gap-20 w-full max-w-[1220px] m-auto text-body-text mt-12"}>
        <div className={"flex flex-col gap-4 w-1/3"}>
          <div className={"flex flex-row items-center gap-3 cursor-pointer"}>
            <Image src={powerUpLogo} alt={"Logo PowerUp"}/>
            <h1 className={"text-body-text text-2xl font-bold"}>PowerUp</h1>
          </div>
          <p className={"font-medium"}>Top up lebih mudah dan tanpa hambatan dengan melakukan top up di PowerUp 🚀</p>
        </div>
        <div className={"flex flex-row w-full"}>
          <ul className={"grid grid-cols-3 gap-16 w-full font-medium basis-3/4"}>
            <li className={"flex flex-col gap-2 w-fit"}>
              <span className={"text-inactive-text"}>LOKASI</span>
